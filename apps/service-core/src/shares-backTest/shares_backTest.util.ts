...
@Injectable()
export default class SharesBackTestUtil {
  constructor(
  ...
  // 通用工具方法：找出两个日期之间的所有交易日（左闭右闭）
  async utilSubTradeDates(
    beginDate: string,
    endDate: string,
  ): Promise<string[]> {
    let endFlag = beginDate === endDate;
    const tradeCalArr = await this.tushareData.getTradeCal();
    if (endFlag) {
      return tradeCalArr[beginDate].is_open ? [beginDate] : [];
    }
    let pointerDate = beginDate;
    const tradeDates: string[] = [];
    while (!endFlag) {
      let curTradeCal = tradeCalArr[pointerDate] as TradeCalResult;
      // 寻找交易日
      while (!curTradeCal.is_open && !endFlag) {
        pointerDate = addDay(curTradeCal.cal_date);
        curTradeCal = tradeCalArr[pointerDate];
        if (pointerDate === endDate) {
          endFlag = true;
        }
      }
      // 找到交易日
      if (!endFlag) {
        ...
        tradeDates.push(tradeDate);
        pointerDate = addDay(tradeDate);
        if (pointerDate === endDate) {
          endFlag = true;
        }
      }
    }
    if (tradeCalArr[endDate].is_open) {
      tradeDates.push(endDate);
    }
    return tradeDates;
  }
